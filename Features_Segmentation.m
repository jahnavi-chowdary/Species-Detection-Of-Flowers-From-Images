function [features_matrix,label] = Features_Segmentation(input,seg,labels_sp,size_sp,edge_sp)
...
% Need to run this loop for all the superpixels i.e from 1:300
...
for i = 0:99  
    ...
    % Getting the label of the Superpixel
    for k = 1:siz
        temp_ind = indices(k,:);
        ind_i = temp_ind(1,1);
        ind_j = temp_ind(1,2);
        if binary(ind_i,ind_j) == 0
            flag0 = flag0 +1;
        else
            flag1 = flag1 +1;
        end
